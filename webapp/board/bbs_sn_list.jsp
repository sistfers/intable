...
<%			
	...
	if(myid != null) {
		...
		total_cnt = dao.do_pag_mine("son_help", myid);	
	} else {
		System.out.println(2);
		// 답변 공지를 제외한 총 게시글의 수는?
		total_cnt = dao.do_pagination("son_help");		
	}
	...
	ArrayList<HashMap<String, String>> n = dao.do_search_notice("son_help");
	ArrayList<HashMap<String, String>> s = null;
	if(myid != null) {
		s = dao.do_search_mine(pageNum, 10, myid, "sonnim", "son_help");
	} else {
		s = dao.do_search(pageNum, 10, "", "", "sonnim", "son_help");
	}	
		...
		<article id="contents_article" class="col-md-12">
			...
			<table class="table table-striped table-hover">
				...
				<tr class="danger" height="40px">
					<td >공지사항</td>						
					<td><a
						href='son_help_control.jsp?action=read
				...
				<tfoot>
					...
					<form method="post" action="son_help_control.jsp?action=rs" name="search">
					<div class="col-xs-10">
				...
				<%
					if (sesson.get("issonnim").equals("true")) {
						...
						%>
						<p align="right"><input type="button" value="상점 게시판" class="btn btn-primary"
							onClick="location.href='store_help_control.jsp?action=page&pn=1'" />
						<input type="button" value="전체 문의" class="btn btn-default" 
							onClick="window.location='son_help_control.jsp?action=page&pn=1'" />
						<input type="button" value="공지" class="btn btn-default"
							onClick="location.href='son_help_control.jsp?action=notice'" />
						<%
						// 손님 글쓰기
						} else {
						%>
						...
						<input type="button" value="전체 문의" class="btn btn-default" 
							onClick="window.location='son_help_control.jsp?action=page&pn=1'" />
						<input type="button" value="내 문의" class="btn btn-default" 
							onClick="window.location='son_help_control.jsp?action=mine&myid=<%=sesson.get("sonnim_id") %>'" />
						<input type="button" value="글쓰기" class="btn btn-default" 
							onClick="window.location='son_help_control.jsp?action=write'" /></p>
						<%
						}						
					} else {
...
<!-- 					<tr>
		...
		<div id="pagination" align="center">			
			<ul class="pagination pagination-sm">
				<li><a href="son_help_control.jsp?action=page&pn=<%=startPage-10 %>">&lt;</a></li>
				<li>
					...
					<a href="son_help_control.jsp?action=page&pn=<%=iCount %>"><%=iCount %></a>				
					<%
						if(iCount < endPage) {
					%>		 
					<%		
