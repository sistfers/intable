...
BoardDao dao = BoardDao.getBoardDao();		
	...
	if(myid != null) {
		...
		total_cnt = dao.do_pag_mine("store_help", myid);	
	} else {
	...
	if(myid != null) {
		s = dao.do_search_mine(pageNum, 10, myid, "store", "store_help");
	} else {
		s = dao.do_search(pageNum, 10, "", "", "store", "store_help");
	}	
			...
			<table class="table table-striped table-hover">
				...
				<%
					if (sesson.get("issonnim").equals("true")) {
						...
						%>
						<p align="right"><input type="button" value="손님 게시판" class="btn btn-warning"
					...
					<input type="button" value="내 문의" class="btn btn-default" 
						onClick="window.location='store_help_control.jsp?action=mine&myid=<%=sesson.get("store_id") %>'" />
